...
type ListResult[T interface{}] struct {
	...
	TotalItems int64 `json:"total_items,omitempty" bson:"total_items"`
	TotalPage  int   `json:"total_page,omitempty" bson:"total_page"`
...
func (q *ListQuery) GetQueryString() string {
	return fmt.Sprintf("page=%v&size=%v&order_by=%s", q.GetPage(), q.GetSize(), q.GetOrderBy())
}
